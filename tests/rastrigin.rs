...
fn test_rastrigin_2d() {
    ...
    assert!(best_energy < EPSILON, 
            "Failed to find global minimum, got {} which is not below epsilon {}", 
            best_energy, EPSILON);
    
    // Print successful result message
    println!("✓ Successfully found solution with energy {} < {}", best_energy, EPSILON);
...
fn test_rastrigin_5d() {
    ...
    assert!(result.best_energy < 1.0, 
            "Failed to find near-global minimum, got {}", result.best_energy);
}
...
fn test_rastrigin_adaptive() {
    ...
    assert!(result.best_energy < 0.5, 
            "Adaptive schedule failed to find good minimum, got {}", result.best_energy);
}
