...
fn test_knapsack_small_known_optimal() {
    ...
    if let Some(optimal_value) = problem.known_optimal_value {
        ...
        println!("Ratio to optimal: {:.2} ({} / {})", ratio, best_value, optimal_value);
        
        // The solution should achieve at least 90% of the optimal value
        assert!(ratio >= 0.9, "Solution does not achieve 90% of optimal value");
    }
...
fn test_knapsack_medium() {
    ...
    println!("Best weight: {} / {}", best_weight, problem.capacity);
    println!("Is valid: {}", is_valid);
    println!("Acceptance ratio: {:.2}%", 100.0 * result.accepted_moves as f64 / result.iterations as f64);
    
    // The solution must be valid
    assert!(is_valid, "Solution exceeds capacity");
    
    // The solution should be better than the initial energy
    assert!(result.best_energy < initial_energy, "Solution did not improve");
}
...
fn test_knapsack_large() {
    ...
    let mut annealer3 = Annealer::new(
        initial_state,
        energy,
        schedule3,
        seeded_rng(SEED),
    ...
    assert!(problem.is_valid(&result1.best_state.selection), "Geometric solution exceeds capacity");
    assert!(problem.is_valid(&result2.best_state.selection), "Logarithmic solution exceeds capacity");
    assert!(problem.is_valid(&result3.best_state.selection), "Adaptive solution exceeds capacity");
