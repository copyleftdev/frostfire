...
fn test_convergence_monotonic() {
    ...
    for &x in &best_state.coords {
        assert!(x.abs() < 1.0, "Solution coordinate {} is not close to origin", x);
    }
}
...
fn test_convergence_different_schedules() {
    ...
    assert!(geo_tracker.has_significant_decrease(0.8), 
            "Geometric schedule did not show significant energy decrease");
    assert!(log_tracker.has_significant_decrease(0.8), 
            "Logarithmic schedule did not show significant energy decrease");
    assert!(adp_tracker.has_significant_decrease(0.8), 
            "Adaptive schedule did not show significant energy decrease");
    
    // Compare convergence rates - which schedule converged fastest
    let geo_final_avg = geo_tracker.moving_averages(20).last().cloned().unwrap_or(f64::INFINITY);
    let log_final_avg = log_tracker.moving_averages(20).last().cloned().unwrap_or(f64::INFINITY);
    let adp_final_avg = adp_tracker.moving_averages(20).last().cloned().unwrap_or(f64::INFINITY);
...
fn test_convergence_restart() {
    ...
    assert!(best_overall_energy < 0.1, 
