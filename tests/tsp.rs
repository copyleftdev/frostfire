...
fn test_tsp_medium() {
    ...
    println!("Improvement: {:.2}%", 100.0 * (initial_energy - result.best_energy) / initial_energy);
    ...
    assert!(result.best_energy < 0.5 * initial_energy, "Solution did not improve significantly");
}
...
fn test_tsp_large() {
    ...
    // The solution should be significantly better than the initial random tour
    assert!(result.best_energy < 0.4 * initial_energy, "Solution did not improve significantly");
}
